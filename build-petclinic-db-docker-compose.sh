...
docker build --build-arg data_directory=${TMP_DATA_DIR} --tag ${DB_IMAGE_NAME}:MD5_$(<build/distributions/spring-petclinic-1.4.2-migration.zip.MD5) --tag ${DB_IMAGE_NAME}:latest --file Dockerfile-db-compose .
